...
{
    class HonestPlayer<MoveT, GameT, GameStateT> : IPlayer<MoveT, GameT, GameStateT>
        where MoveT : Enum
        where GameT : IGame<MoveT, GameStateT>
        where GameStateT : IGameState<MoveT>
    {
        readonly Random random = new Random();
        readonly HonestStrategy<MoveT, GameT, GameStateT> strategy;
        ...
        public HonestPlayer(HonestStrategy<MoveT, GameT, GameStateT> strategy = null)
        {
            this.strategy = strategy ?? new HonestStrategy<MoveT, GameT, GameStateT>(
        ...
        where GameStateT : IGameState<MoveT>
    {
        ...
        private IEnumerable<(GameStateT state, double weight)> GetBeliefsInRealState(GameT game, GameStateT state, int playerId)
        {
            ...
            var simulatedPlayer = new HonestPlayer<MoveT, GameT, GameStateT>(
                new HonestStrategy<MoveT, GameT, GameStateT>(moveValueCache, beliefsInStateCache, bestMoveCache));
                                            ...
                                            List<Func<GameStateT, bool>> claimsSent, List<Func<GameStateT, bool>> claimsReceived)
        {
            ...
            // If the claim sent is false to the current state, return 0 utilities
            ...
            for (int playerIndex = 0; playerIndex < game.NumberOfPlayers; playerIndex++)
            {
                ...
                for (int playerMove = 0; playerMove < possibleMovesByPlayer[playerIndex].Count(); playerMove++)
                {
                    ...
                    // Iterate through all claims made, trust all of them
